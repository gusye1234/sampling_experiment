...
class LightGCN(nn.Module):
    ...
    def get_user_item_embedding(self):
        with torch.no_grad():
            ...
            users_emb = self.soft(all_users)
            items_emb = self.sig(all_items)
...
class LightGCN_xij_item_personal_single(nn.Module):
    ...
    def __init_weight(self):
        ...
        self.latent_dim = self.config['latent_dim_var']
        self.xij_dim = self.config['xij_dim']
        self.hyper_x = self.config['hpyer_x']
        self.embedding_user = torch.nn.Embedding(num_embeddings=self.num_users, embedding_dim=self.latent_dim)
        self.embedding_item = torch.nn.Embedding(num_embeddings=self.num_items, embedding_dim=self.latent_dim)
        nn.init.xavier_uniform_(self.embedding_user.weight, gain=1)
        nn.init.xavier_uniform_(self.embedding_item.weight, gain=1)
...
class LightGCN_xij_item_personal_outer_product_single(nn.Module):
    def __init__(self, config):
        super(LightGCN_xij_item_personal_outer_product_single, self).__init__()
        self.config = config
        self.dataset : dataloader.BasicDataset = config['dataset']
        self.sig = nn.Sigmoid()
        self.soft = nn.Softmax(dim=1)
        self.__init_weight()
        print('LightGCN_xij_item_personal_outer_product init!!!')
    ...
    def computer(self):
        ...
        if self.drop:
            ...
            g_droped = self.Graph
        for layer in range(self.n_layers):
            all_emb = torch.sparse.mm(g_droped, all_emb) + torch.sparse.mm(g_droped, all_emb) * all_emb
            embs.append(all_emb)
        embs = torch.stack(embs, dim=1)
        # print(embs.size())
        light_out = torch.mean(embs, dim=1)
...
class LightGCN_xij_item_personal_outer_product_matrix(nn.Module):
    def __init__(self, config):
        super(LightGCN_xij_item_personal_outer_product_matrix, self).__init__()
        self.config = config
        self.dataset : dataloader.BasicDataset = config['dataset']
        self.sig = nn.Sigmoid()
        self.soft = nn.Softmax(dim=1)
        self.__init_weight()
        print('LightGCN_xij_item_personal_outer_product_matrix init!!!')
...
class LightGCN_xij_item_personal_matrix(nn.Module):
    ...
    def forwardWithReg(self, users, items, xij):
        ...
        print('decay', self.weight_decay1)
        #reg_loss = reg_loss / float(len(users))
