...
class LastFM(BasicDataset):
    ...
    def getAllUserPosItems(self):
        ...
        for user in users:
            positems = torch.tensor(self.UserItemNet[user].nonzero()[1]).long()
            posusers = torch.tensor([user]*len(positems)).long()
        ...
        return staPosUsers, staPosItems, torch.tensor(numPosUsers)
...
class Ciao(BasicDataset):
    ...
    def __init__(self, path="../data/ciao"):
        ...
        self.testItem = testData[:, 1]
        self.Graph = None
        print(f"Ciao Sparsity : {(len(self.trainUser) + len(self.testUser)) / self.n_users / self.m_items}")
